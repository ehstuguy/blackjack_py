...
class Shoe:
    ...
    def __init__(self, nDecks: int):
        """Any number of decks works, but 6 is most commoly used"""
        self.makeShoe(nDecks)
        self.cutDeck()


    def makeShoe(self, nDecks: int):
        suits = ["♠", "♥", "♦", "♣"]
...
class card:
    ...
    def __init__(self, cardInfo: tuple):
...
class Player:
    ...
    def __init__(self, bankroll: int, playerNum: int):
        """How much the player starts with and their seat number"""
...
class hand:
    ...
    def __init__(self, nHand: object, playerInfo: dict):
    ...
    def addCard(self, nCard: object, pData: dict):
...
def decision(opts: list[str], dType: str, incompetent: int =0):
...
def playDecision(Plr: object, hnd: object, hndNum: int, shoe: object):
    while hnd.Stand == False and hnd.Bust == False:
        ...
        if hnd.decision == "hit":
            hnd.addCard(shoe.cards[0], Plr.info)
            del shoe.cards[0]
            ...
            for crd in hnd.cHand:
                splitHand = hand([crd, shoe.cards[0]], Plr.info)
                del shoe.cards[0]
                Plr.hands.insert(hndNum, splitHand)
            Plr.hands.pop(hndNum-1)
            hnd = Plr.hands[0]
        if hnd.Bust == True:
            print(f"\nBUST!!\n\n{hnd.cInfo}\nvalue: {hnd.cSum}"
                  f"\n{'='*50}")
...
def playGame(shoe: object, tableList: list, playerList: list):
    ...
    for currPlayer in playerList:
        ...
        while hndNum <= len(currPlayer.hands):
            for thisHand in currPlayer.hands:
                playDecision(currPlayer, thisHand, hndNum, shoe)
            hndNum = hndNum + 1
...
if __name__ == "__main__":
    ...
    # Got to test for pair of aces natty ('♦', 'A'), ('♦', 'A') -> 22 ??
